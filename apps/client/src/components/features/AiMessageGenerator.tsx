...
export const AiMessageGenerator = ({ application }: AiMessageGeneratorProps) => {
    ...
    const generateMessage = () => {
        ...
        setTimeout(() => {
            const template = `Bonjour,\n\nJe suis très intéressé par le poste de ${application.position} chez ${application.company} que j'ai vu sur ${application.source || 'votre site'}.\n\nMon profil semble correspondre à vos attentes, notamment sur les aspects techniques évoqués dans l'offre. ${application.location ? `Ã‰tant basé à ${application.location}, je` : 'Je'} serais ravi d'échanger avec vous sur cette opportunité.\n\nBien cordialement,`;
            setMessage(template);
