...
export const extractJobDetailsFromText = async (text: string): Promise<any> => {
    ...
    for (const modelName of modelsToTry) {
        try {
            ...
            console.warn(`[SDK] Ã‰chec avec le modèle ${modelName}:`, error.message);
            lastError = error;
        }
    }
    ...
    try {
        ...
        const expRegex2 = /\b(\d+(?:\s?-\s?\d+)?)\s*(?:an|ans|année|années)\s*d['â€™]?\s*expérience/i; // 2 ans d'expérience
        ...
        // Extract Industry
        let industry = "";
        const industryMatch = text.match(/Secteur\s*d['â€™]?\s*activité\s*:\s*([^\n]+)/i);
    ...
    throw new Error(`Ã‰chec total de l'analyse (IA & Offline). Vérifiez votre clé API ou le texte fourni.`);
};
