...
async function callLocalAI(messages: { role: string; content: string }[], config: LocalAIConfig, jsonMode: boolean = true): Promise<string> {
    ...
    if (typeof window !== 'undefined' && window.location.protocol === 'https:' && config.baseUrl.startsWith('http:')) {
        throw new Error(
            "ðŸ”’ Blocage Sécurité Navigateur (Mixed Content) :\n" +
    ...
    try {
        ...
        try {
            ...
            if (error.name === 'AbortError') {
                throw new Error("â³ Timeout (90s): L'IA met trop de temps à répondre. Essayez un modèle plus petit (ex: llama3.1:8b) ou vérifiez votre PC.");
            }
            throw error;
        }
    } catch (error: any) {
