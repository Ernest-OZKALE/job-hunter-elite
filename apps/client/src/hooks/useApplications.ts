...
export const useApplications = (userId: string | undefined) => {
    ...
    useEffect(() => {
        ...
        // Abonnement aux changements en temps réel via Supabase
    ...
    const addApplication = async (data: Omit<JobApplication, 'id'>) => {
        ...
        // Mise à jour optimiste (nécessite un ID temporaire ou recharge, ici rechargement préféré)
        const { data: returnedData, error } = await supabase
            .from('applications')
            .insert([dbData])
            .select() // Retourne les données pour récupérer le nouvel ID
            .single();
